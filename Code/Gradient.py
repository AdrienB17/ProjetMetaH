...
def  show_solution(partition):
    '''
        Affiche la solution
    '''
    print(f'Partition 1 : {[v for v in range(len(partition)) if partition[v] == 0]}')
    print(f'Partition 2 : {[v for v in range(len(partition)) if partition[v] == 1]}')


def swap(tab, a, b):
    tab[a], tab[b] = tab[b], tab[a]

def neighbourhoodSwap(partition):
    '''
        Debug.
...
def neighbourhoodPnD(partition):
    '''
        Debug.
        Cette fonction affiche le voisinage d'une configuraion donnée.
...
def gradient(graph, param):
    ...
    classSizes = [sum(C0) , len(C0) - sum(C0)]
